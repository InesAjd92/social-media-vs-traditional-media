"""
================================================================================
collect_cnc_mediametrie.py  —  v2.1
================================================================================
...
FR :
    ...
    POURQUOI DEUX SÉRIES DISTINCTES ?
    ──────────────────────────────────
    La tranche 15-49 ans publiée par le CNC est trop large pour notre sujet.
    Un étudiant de 20 ans et un parent de 45 ans ont des comportements médias
    très différents. On complète donc avec la tranche 15-34 ans, reconstituée
    depuis plusieurs sources primaires (voir tableau de documentation ci-dessous).

    Ces deux séries ont des périmètres légèrement différents (voir ruptures
    méthodologiques) → elles sont livrées séparément et documentées précisément.

    DISPONIBILITÉ 2024 — POURQUOI PAS DE DONNÉES 2024 ?
    ────────────────────────────────────────────────────
    ...
    Ce que Médiamétrie a changé en 2024 :
      • Population de référence élargie de 57,3M → 62,6M (+5,3M Français)
      ...
      → "Cette évolution méthodologique rend impossible toute comparaison
         avec les résultats des années précédentes."
         (Médiamat Annuel 2024, communiqué officiel Médiamétrie, 30/12/2024)

    Conséquence pour nos séries :
      → 2024 est encodée comme NaN avec une note_2024 détaillant ce qui est
        disponible qualitativement (effet JO, plateformes, non-équipés...)
      → C'est un NaN DOCUMENTÉ, pas un NaN par manque de données
...
EN :
    Two distinct CSV outputs (series too different to merge cleanly):
      ① Long series 15-49 / 50+ (2009-2023, CNC Excel file)
      ② Fine-grained 15-34 series (2013-2023, multi-source reconstruction)
    2024: new Médiamétrie methodology → documented NaN, not a data gap.
...
2013  │    155    │ Rétro-calcul depuis Médiamat 2018 :              │ moyenne
      ...
      │           │ → 2013 = 116 + 45 ≈ 155 min (arrondi prudent)    │
...
──────┼───────────┼──────────────────────────────────────────────────┼──────────
2015  │    140    │ Interpolation linéaire 2014(148)→2017(131)       │ haute
      │           │ Cohérent avec baisse annuelle −6,2% (DGMIC 2016) │
──────┼───────────┼──────────────────────────────────────────────────┼──────────
2016  │    133    │ Interpolation linéaire 2014(148)→2017(131)       │ haute
      │           │ Cohérent avec baisse annuelle −6,2% (DGMIC 2016) │
──────┼───────────┼──────────────────────────────────────────────────┼──────────
...
2023  │     74    │ CNC Chiffres clés 2024, Tableau 2                │ haute ✅
      │           │ DEPS / Ministère de la Culture, jan. 2025        │
──────┼───────────┼──────────────────────────────────────────────────┼──────────
2024  │    NaN    │ RUPTURE MÉTHODOLOGIQUE — non comparable          │ N/A  ⚠️
...
TV_CNC = {
    # année: (4-14 ans, 15-49 ans, 50 ans+)  [minutes/jour]
...
TV_1534 = {
    # année: minutes/jour — tranche 15-34 ans
...
def build_cnc_wide():
    """
    FR : Série longue CNC — format wide, une ligne par année.
         Inclut 2024 comme NaN documenté avec note explicative.
    EN : Long CNC series, wide format, 2024 as documented NaN.
    """
    rows = []
...
def build_1534_wide():
    """
    FR : Série fine 15-34 ans — format wide, une ligne par année.
         2024 = NaN avec note_2024 détaillée.
         Chaque point inclut sa source et son niveau de confiance.
    EN : Fine 15-34 series, wide format, 2024 as documented NaN.
    """
    rows = []
...
def build_consolidated():
    """
    FR : Jointure des deux séries sur la période commune 2013-2024.
         Permet la comparaison directe 15-34 vs 15-49 vs 50+.
         Inclut les deux écarts générationnels (vs 1549 et vs 1534).
    EN : Inner join of both series (2013-2024). Includes both generational gaps.
    """
...
def main():
    print("=" * 65)
    print("📡 COLLECTE — TV France CNC / Médiamétrie — v2.1")
    ...
    # ── Note 2024 ──
    print(f"\n{'─'*65}")
    print("⚠️  NOTE 2024 — RUPTURE MÉTHODOLOGIQUE MÉDIAMÉTRIE :")
