...
On adopte donc une stratégie hybdride en 2 parties :

PARTIE 1 : Téléchargement automatique via l'API d'OWID des données suivantes :
      - Utilisateurs d'internet par pays (Banque Mondiale / ITU)
      - Utilisateurs de réseaux sociaux par plateforme 
      - Temps passé sur les médias numériques aux USA (eMarketer)

PARTIE 2 : Reconstruction manuelle des données depuis d'autres sources 
      - Temps passé sur les réseaux sociaux par pays 2012-2024
        (Source : GWI via DataReportal / Statista - données publiées annuellement)
Ces chiffres proviennent de rapports académiques et professionnels largement cités dans la littérature scientifique sur les médias numériques.

## Principe de reproductibilité : ##

Toutes les données sont sauvegardées dans leur état brut (raw/)
avec un horodatage, avant tout traitement. Cela garantit que l'analyse 
peut être reproduite exactement à l'identique.

================================================================================
Auteure / Author : Inès Amdjahed - Data analyst junior
...
OUTPUT_DIR = os.path.join(os.path.dirname(__file__), '..', '02_data', 'raw') # dossier où seront enregistrées les raw data (données brutes)
...
# ════════════════════════════════════════════════════════════════════════════════
# PARTIE 1 : DATASETS OWID TÉLÉCHARGEABLES EN CSV
# ════════════════════════════════════════════════════════════════════════════════
# Ces URLs ont été validées manuellement sur ourworldindata.org.
# Format API : https://ourworldindata.org/grapher/[slug].csv
...
# Pertinence : variable de contexte - mesure la pénétration numérique nécessaire pour comprendre l'exposition possible aux réseaux sociaux
...
# Dataset 2 : Nombre d'utilisateurs d'internet (en absolu) ──────────────
# Source originale : Banque Mondiale / ITU
# Variable : nombre total d'internautes en millions par pays
# Couverture : ~200 pays, 1990-2023
# Pertinence pour H3 : mesure la croissance de l'audience numérique potentielle pays par pays dans le temps
...
# Pertinence pour H1 : mesure directe du temps d'écran numérique total
# et permets de visualiser la substitution médias traditionnels -> numériques
...
#
# Ces chiffres sont cités dans la littérature académique sur les médias
...
# url : https://www.statista.com/statistics/433871/daily-social-media-usage-worldwide/?srsltid=AfmBOorA-D3GCGh1FZiCmVAuoz2V4bE1W5mx33XO6aX6b3YZqGacKpus
...
GWI_SOCIAL_MEDIA_GLOBAL = {
    ...
    2021: 147,  # ~2h27 - TikTok explose, réels Instagram lancés
...
# Données plateformes sociales - Monthly active users (MAU) historique ─────────────────────────────
...
# Ces chiffres proviennent des rapports trimestriels officiels des plateformes (earnings reports, investor relations) 
# et sont cités dans la littérature académique sur les médias numériques.
#
# Sources : rapports officiels Meta, ByteDance, Snap, X Corp compilés via DataReportal / Statista / Hootsuite
...
"""
Pour nous faciliter la suite, on va créer une fonction qui télécharge un dataset OWID depuis son URL CSV directe.
La fonction retournera un DataFrame pandas, ou None si le téléchargement échoue.

Gestion des erreurs HTTP :
- 404 : signifie que le slug OWID n'existe pas / a changé. Solution : vérifier sur le site
- 429 : signifie un rate limiting i.e. trop de requêtes. Solution : attendre et réessayer
- 5xx : signifie une erreur côté serveur OWID. Solution : réessayer plus tard
- ConnectionError : pas de connexion internet

Paramètres : 
name (str) : Le nom de la variable ou du dataset (ex. "internet_users_by_country"), utilisé pour l’affichage.

config (dict) : Un dictionnaire contenant au moins ces clés :

'url' : l’URL du fichier CSV à télécharger
'description_fr' : description en français
'description_en' : description en anglais
'source' : la source des données

Nous utiliserons également un User Agent par bonne pratique pour éviter d'être bloqué sur nos requêtes.
"""


def download_owid_dataset(name: str, config: dict) -> pd.DataFrame | None: # télécharge un fichier csv depuis url donnée dans la configuration puis le charge dans un tableau de données pandas
    print(f"\n{'─'*60}")
    ...
    try:
    # User-Agent qui identifie le script comme un projet académique 
    ...
    # StringIO transforme le texte CSV reçu en "fichier virtuel"
    # que pandas peut lire directement sans écrire sur le disque
        df = pd.read_csv(StringIO(response.text))

    # Détection automatique des colonnes clés : OWID utilise parfois 'Year', parfois 'year' selon les datasets
    ...
    # Audit des valeurs manquantes (NaN = donnée non disponible pour ce pays/année)
        missing     = df.isnull().sum().sum() # nombre total de NaN dans tous le df
        pct_missing = missing / (df.shape[0] * df.shape[1]) * 100 # % de NaN / total des données
        print(f"   Valeurs manquantes : {missing} ({pct_missing:.1f}%)")
...
"""
Nous allons maintenant créer une fonction qui construit les datasets GWI à partir des données reconstruites manuellement.
On transforme les dictionnaires Python en DataFrames au format "long" :
    une ligne = une observation (année, pays, valeur).
C'est le format standard pour les analyses en panel et les visualisations 
avec seaborn/matplotlib.

"""

def build_gwi_datasets() -> dict:
  
# Dataset GWI 1 : Tendance mondiale du temps passé sur les réseaux sociaux (2012-2024)
...
"""
Cette fonction permet de sauvegarder un DataFrame en CSV directement dans le dossier 02_data/raw/.

Convention de nommage : {prefix}_{name}_{date}.csv
Exemples : owid_internet_users_by_country_2026-03-11.csv
        gwi_social_media_global_trend_2026-03-11.csv

Règle importante : les fichiers raw/ ne sont jamais modifiés.
Toutes les transformations se feront dans les notebooks de cleaning.
C'est un principe essentiel de reproductibilité scientifique.

Paramètres : 
df : le DataFrame pandas à sauvegarder.
name : nom de base du fichier
prefix (optionnel) : chaîne ajoutée en début du nom pour catégoriser (ex. "gwi").
...
# encoding utf-8 : standard universel, gère les accents et caractères spéciaux
    ...
    return filepath # pour avoir le chemin où le fichier est sauvegardé

# Fonction 4 # 

"""
Pour se préparer à l'EDA, on prépare une fonction qui réalise une inspection préliminaire (data profiling).
...
"""
Cette fonction pilote l’ensemble du processus de collecte et de sauvegarde des données.
...
def main():
    ...
    log_lines = [    # lignes du fichier de log (pour documenter la provenance)
    ...
    for name, config in OWID_DATASETS.items():
        ...
        if df is not None: # si df n'est pas vide
...
# Permet de savoir exactement quelles données ont été collectées, quand, depuis où ?
    ...
    print(f"\n{'='*60}")
    print("   Prochaine étape : 03_notebooks/01_cleaning_wrangling.ipynb")
    print(f"{'='*60}\n")
